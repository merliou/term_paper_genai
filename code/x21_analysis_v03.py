...
**Version 3.0:** Enthält eine zusätzliche Detailanalyse für Lidl im
Ländervergleich.
"""
...
OUTPUT_DIR = 'analyse_ergebnisse_v03/'
...
def generate_visual_report(df):
    ...
    plt.figure(figsize=(12, max(8, num_supermarkets * 0.4)))
    ax = sns.barplot(x='alc_prod_ratio', y='supermarket', data=all_supermarkets_sorted, orient='h', palette='magma')
    ax.set_title('Anteil alkoholischer Produkte nach Supermarkt (Alle)', fontsize=16, pad=20)
    ...
    if df_lidl.empty:
        ...
        lidl_page_presence = df_lidl.groupby('country')['alc'].value_counts(normalize=True).mul(100).rename('percent').reset_index()
        ...
        lidl_prod_summary = df_lidl.groupby('country')[['prod_pp', 'prod_alc']].sum()
        ...
        ax.set_title('Lidl im Ländervergleich: Alkoholpräsenz vs. Produktanteil', fontsize=16, pad=20)
        ax.set_xlabel('Land', fontsize=12)
        ax.set_ylabel('Anteil in Prozent (%)', fontsize=12)
        ...
        if not df_lidl_alc.empty:
            ...
            ax.set_title('Lidl im Ländervergleich: Platzierung der Alkoholwerbung', fontsize=16, pad=20)
            ax.set_xlabel('Land', fontsize=12)
            ax.set_ylabel('Relative Seitenposition (0=Anfang, 1=Ende)', fontsize=12)
            plt.tight_layout()
