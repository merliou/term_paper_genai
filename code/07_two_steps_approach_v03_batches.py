...
PROCESSING_CSV_FILE = 'annotations/subset_1_annotated_twosteps_v03.csv'
BASE_CSV_FILE = 'subsets/subset_1.csv' # Nur als Quelle, falls die PROCESSING_CSV_FILE noch nicht existiert

# Konfiguration für Schritt 2 & 3: Batch-Verarbeitung
TEXT_BATCH_SIZE = 10 # Wie viele Texte pro Durchgang an das Text-LLM gesendet werden
IMAGE_BATCH_SIZE = 3 # Wie viele Bilder pro Durchgang an das LLM gesendet werden

# Ollama Konfiguration
OLLAMA_ENDPOINT = "http://localhost:11434/api/generate"
TEXT_MODEL = "qwen2.5vl:3b"
IMAGE_MODEL = "qwen2.5vl:3b"
API_TIMEOUT = 1320 # Timeout für API-Aufrufe in Sekunden

# Pfade zu den Prompt-Dateien
TEXT_PROMPT_PATH = "term_paper_genai/prompts/01_text_classification_batch_prompt.txt" # NEUER PROMPT
...
def step2_classify_text_in_batches(df):
    """Klassifiziert Texte effizient in Batches."""
    ...
    # Initialisiere die Flag-Spalte
    df['alc_keyword_flag'] = 0
    
    # Finde die Zeilen, die verarbeitet werden müssen (Text vorhanden)
    ...
    for i in range(0, total_to_process, TEXT_BATCH_SIZE):
        ...
        # Bereite den Input für den Batch-Prompt vor
        batch_data = []
        for index in batch_indices:
            # Wir verwenden den DataFrame-Index als eindeutige ID
            batch_data.append({"ID": index, "TEXT": df.loc[index, 'extracted_text']})
        
        prompt = text_prompt_template.replace("{batch_of_texts}", json.dumps(batch_data, indent=2))
        
        # Rufe die API für den ganzen Batch auf
        result = call_ollama_api(prompt, TEXT_MODEL)
        
        # Verarbeite das Ergebnis
        if "results" in result and isinstance(result["results"], list):
            for item in result["results"]:
                # Finde die richtige Zeile über die ID und update den Flag
...
def step3_annotate_images_in_batches(df):
    ...
    annotation_cols = ['alc', 'product', 'reduc', 'prod_pp', 'prod_pp_alc'] # Ihre angepasste Spaltenliste
...
if __name__ == "__main__":
    ...
    if 'alc_keyword_flag' not in df.columns:
        df = step2_classify_text_in_batches(df) # GEÄNDERTER FUNKTIONSAUFRUF
        df.to_csv(PROCESSING_CSV_FILE, index=False, encoding='utf-8-sig')
    else:
