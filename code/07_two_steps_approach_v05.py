...
# ==============================================================================
PROCESSING_CSV_FILE = 'annotations/subset_1_annotated_twosteps_v05.csv'
BASE_CSV_FILE = 'subsets/subset_1.csv'

IMAGE_BATCH_SIZE = 3

OLLAMA_ENDPOINT = "http://localhost:11434/api/generate"
# *** WICHTIG: Dediziertes, schnelles Text-Modell für Schritt 2 ***
TEXT_MODEL = "mistral:7b-instruct-v0.3-q4_K_M" 
IMAGE_MODEL = "qwen2.5vl:3b" # Ihr multimodales Modell für Schritt 3
API_TIMEOUT = 660

# *** WICHTIG: Passen Sie den Pfad zu Ihrem Batch-Prompt an ***
# Wir verwenden wieder einen einfachen Prompt, da wir Seite für Seite verarbeiten
...
# ==============================================================================
# --- WORKFLOW-SCHRITTE (SCHRITT 2 ZURÜCK ZU SEQUENZIELL) ---
# ==============================================================================
...
def step2_classify_text_sequentially(df):
    """Klassifiziert Texte schnell und robust, Seite für Seite."""
    print("\n--- SCHRITT 2: Klassifiziere Texte auf Alkohol-Stichworte (Sequenzieller Modus) ---")
    
    # Stellen Sie sicher, dass Sie den richtigen (einfachen) Prompt für diese Funktion verwenden
    # Er sollte so aussehen wie der allererste Text-Prompt, der nur "{page_text}" erwartet
    ...
    for index, row in df.iterrows():
        ...
        # *** NEUE SICHERHEITSMASSNAHME: Text kürzen ***
        truncated_text = text[:3000] # Nur die ersten 3000 Zeichen verwenden
        ...
        # Keine Pause (time.sleep) nötig, da die API-Calls selbst Latenz haben
...
def step3_annotate_images_in_batches(df):
    ...
    for i in range(0, len(to_process_indices), IMAGE_BATCH_SIZE):
        ...
        df.to_csv(PROCESSING_CSV_FILE, index=False, encoding='utf-8-sig') # *** KORREKTUR DES TIPPFEHLERS ***
...
if __name__ == "__main__":
    # *** ÄNDERUNG: Vor dem Ausführen des Skripts, wird die alte fehlerhafte CSV gelöscht, um neu zu beginnen ***
    # Wir löschen die Arbeitsdatei, da Schritt 1 und 2 neu und sauber ausgeführt werden müssen.
    ...
    # Schritt 1 & 2 werden jetzt sicher neu ausgeführt
    ...
    if 'alc_keyword_flag' not in df.columns:
        df = step2_classify_text_sequentially(df) # GEÄNDERTER, ROBUSTER FUNKTIONSAUFRUF
        df.to_csv(PROCESSING_CSV_FILE, index=False, encoding='utf-8-sig') # *** KORREKTUR DES TIPPFEHLERS ***
    else:
